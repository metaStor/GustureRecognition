...
def load_parameters():
    ...
    with tf.Session() as sess:
        # saver.restore(sess, "model_2500/cnn_model.ckpt")
        saver.restore(sess, tf.train.latest_checkpoint("model/model_2000"))
        # print(W_conv1.eval())
...
def display_result(mat, prediction):
    ...
    font = ImageFont.truetype('/usr/share/fonts/truetype/方正少儿GBK.ttf', 150)
    draw.text((100, 100), "识别结果: {}".format(str(prediction)), fill='#FF0000', font=font)
    im.show()


if __name__ == "__main__":
    mat, new_mat = img_to_mat("000562.jpg")
